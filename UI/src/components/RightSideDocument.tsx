...
export const RightSideDocument: React.FC<RightSideDocumentProps> = ({
  ...
  return (
    <div className="h-full flex-1 flex flex-col overflow-hidden">
      ...
      <div
        className={`h-max flex items-end border-b gap-1 shrink-0 ${
          darkMode
      ...
      <div
        className={`flex-1 relative overflow-hidden p-6 flex flex-col gap-4 ${
          darkMode ? "bg-zinc-950" : "bg-zinc-50"
        }`}
      >
        {currentPage && (
          <div className="relative w-full max-w-2xl flex flex-col group pb-2">
            <input
              type="text"
              value={localTitle}
            ...
            <span
              className={`absolute bottom-8 left-0 h-[1.5px] w-full transition-colors ${
                darkMode ? "bg-zinc-800" : "bg-zinc-200"
              }`}
            />

            <span
              className={`absolute bottom-8 left-0 h-[1.5px] w-full transition-transform duration-300 origin-left scale-x-0 group-focus-within:scale-x-100 ${
